...
states.game = {
	...
	"_init" : function(message){
		...
		$('#game').delegate("#shuffleLetters", "click.shuffleLetters", function(e){
			
			e.preventDefault();
			
			var $tiles = $('#input .tiles li');
			
			// make all position: absolute
			
			$tiles.reverse().each(function(){
				var $this = $(this);
			...
			while ($tiles.length > 1){
				...
				var pair = {};
				pair.a = $tiles.splice(0,1);
				var index = Math.round(Math.random() * $tiles.length-1);
				pair.b = $tiles.splice(index,1);
			...
			var animatePair = function(i, callback){
				...
				$a.animate({"top": '-=15'},50).animate({'left':posB.left}, 160).animate({"top": '+=15'}, 50);
				$b.animate({"top": '-=15'},50).animate({'left':posA.left}, 160).animate({"top": '+=15'}, 50,function(){
			...
			animatePair(0, doneFunc);		
			
		});
		
		
		$('#game').delegate("#clearLetters", "click.clearLetters", function(e){
		
			e.preventDefault();
		
			$('#output .tiles li').appendTo($('#input .tiles'));
	...
	"_end" : function(message){
		...
		$('#game').undelegate("#shuffleLetters", "click.shuffleLetters");
		$('#game').undelegate("#clearLetters", "click.clearLetters");
